...
int PX_NewHandle(const char * clsName, int fd)
{
  ...
  if (fd > highWaterMark) {
    ...
    if (fd < (maxHandles-maxHandles/20))
      PTRACE(1, "PWLib\tFile handle high water mark set: " << fd << ' ' << clsName);
    else
      PTRACE(0, "PWLib\tFile handle high water mark within 5% of maximum: " << fd << ' ' << clsName);
  }

  if (fd < lowWaterMark) {
    lowWaterMark = fd;
    PTRACE(1, "PWLib\tFile handle low water mark set: " << fd << ' ' << clsName);
  }
